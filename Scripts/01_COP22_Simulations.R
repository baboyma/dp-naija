...
#' @title Clean Mechs
#'
#' @note TODO: Move this to an input csv file
#'
...
add_assumptions <- function(df_results, assums) {
  ...
  df_results %>%
    left_join(assums$hvol, by = c("psnu", "orgunituid", "sitename")) %>%
    left_join(assums$gf, by = c("psnu" = "state", "sitename" = "orgunit")) %>%
...
# DATA ----
  ...
  # Indicator with age/sex disaggs ----
  df_disaggs <- df_sites %>%
  ...
  df_sites %>%
    ...
    full_join(df_disaggs,
              by = c("indicator", "standardizeddisaggregate" = "disagg"))
